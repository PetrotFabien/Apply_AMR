...
def create_app():
    ...
    def init_db():
        ...
        # Soft migration: add columns if they don't exist (idempotent)
        ...
        if count == 0:
            for i in range(1, 45):
                code = f"SOL-{i:02d}"
                db.execute(
                    "INSERT INTO location(code,name,kind,capacity) VALUES (?,?, 'SOL', 1)",
                    (code, f"Emplacement {code}"),
                )
            for i in range(1, 51):
                code = f"ETG-{i:02d}"
                db.execute(
                    "INSERT INTO location(code,name,kind,capacity) VALUES (?,?, 'ETAGERE', 1)",
                    (code, f"Etagère {code}"),
                )
            posts = [
            ...
            for code, name in posts:
                db.execute(
                    "INSERT INTO location(code,name,kind,capacity) VALUES (?,?, 'POSTE', NULL)",
                    (code, name),
                )
            db.commit()
    ...
    def count_items_in_location(loc_id):
        return (
            get_db()
            .execute("SELECT COUNT(*) AS c FROM item WHERE location_id=?", (loc_id,))
            .fetchone()["c"]
        )
    ...
    def move_item(item_id, to_location_id, action="MOVE", user=None):
        ...
        if not loc:
            abort(400)
        if loc["capacity"] == 1:
            occ = count_items_in_location(loc["id"])
            if occ >= 1:
                raise ValueError(f"Emplacement {loc['code']} occupé")
        db.execute(
            "INSERT INTO movement(item_id, from_location_id, to_location_id, action, user) VALUES (?,?,?,?,?)",
            (item_id, it["location_id"], to_location_id, action, user),
        )
        new_status = it["status"]
        if loc["kind"] == "POSTE":
            if loc["code"] == "POSTE-PHOTO":
                new_status = "PHOTO"
            elif loc["code"] == "POSTE-INSPECTION":
                new_status = "INSPECTION"
            elif loc["code"] == "POSTE-EMBALLAGE":
                new_status = "EMBALLAGE"
    ...
    def index():
        ...
        for kind in ("SOL", "ETAGERE"):
            ...
            occ = db.execute(
                """
                SELECT COUNT(*) AS c
                FROM item i JOIN location l ON i.location_id=l.id
                WHERE l.kind=? AND l.capacity=1
                """,
                (kind,),
            ).fetchone()["c"]
    ...
    def items():
        ...
        if request.method == "POST":
            ...
            db.commit()
            flash(f"Article {sku} créé (id {new_id}).", "ok")
    ...
    def item_detail(item_id):
        ...
        if not it:
            abort(404)
        loc = (
            db.execute("SELECT * FROM location WHERE id=?", (it["location_id"],)).fetchone()
            if it["location_id"]
            else None
        )
        moves = db.execute(
            """
            SELECT m.*, lf.code AS from_code, lt.code AS to_code
            FROM movement m
        ...
        sol_free = free_slots("SOL")
        etg_free = free_slots("ETAGERE")
    ...
    def move(item_id):
        ...
        try:
            move_item(item_id, to_id, action="MOVE")
            flash("Mouvement effectué.", "ok")
    ...
    def update_refs(item_id):
        ...
        db.commit()
        flash("Références mises à jour.", "ok")
        return redirect(url_for("item_detail", item_id=item_id))
    ...
    def send_to_photo(item_id):
        poste = location_by_code("POSTE-PHOTO")
        move_item(item_id, poste["id"], action="TO_PHOTO")
        flash("Envoyé au poste Photo.", "ok")
        return redirect(url_for("item_detail", item_id=item_id))
    ...
    def upload_photo(item_id):
        ...
        if not file or file.filename == "":
            flash("Aucun fichier donné.", "error")
            return redirect(url_for("item_detail", item_id=item_id))
        ext = os.path.splitext(file.filename)[1].lower()
        if ext not in [".jpg", ".jpeg", ".png", ".webp"]:
            flash("Format non supporté (jpg, png, webp).", "error")
            return redirect(url_for("item_detail", item_id=item_id))
        fname = f"item_{item_id}_{int(datetime.now().timestamp())}{ext}"
        path = os.path.join(UPLOAD_DIR, fname)
        file.save(path)
        db = get_db()
        db.execute("UPDATE item SET photo_path=?, status='PHOTO' WHERE id=?", (fname, item_id))
        db.commit()
        flash("Photo enregistrée.", "ok")
        return redirect(url_for("item_detail", item_id=item_id))
    ...
    def work_inspection():
        ...
        if request.method == "POST":
            ...
            if result == "OK":
                poste = location_by_code("POSTE-EMBALLAGE")
                move_item(item_id, poste["id"], action="INSPECT_OK")
                flash("Inspection OK → vers Emballage.", "ok")
            else:
                poste = location_by_code("POSTE-INSPECTION")
                move_item(item_id, poste["id"], action="INSPECT_NOK")
                flash("Inspection NOK (reste au poste).", "error")
    ...
    def work_emballage():
        ...
        if request.method == "POST":
            item_id = int(request.form.get("item_id"))
            target_kind = request.form.get("target_kind")  # SOL / ETAGERE
            slot_id = request.form.get("slot_id")
            if not slot_id:
                slot = first_free_slot(target_kind)
                if not slot:
                    flash(f"Aucun emplacement {target_kind} libre.", "error")
                    return redirect(url_for("work_emballage"))
                slot_id = slot["id"]
            try:
                move_item(item_id, int(slot_id), action="PUT_STOCK")
                db.execute("UPDATE item SET status='STOCK' WHERE id=?", (item_id,))
                db.commit()
                flash("Article stocké.", "ok")
            except ValueError as e:
                flash(str(e), "error")
            return redirect(url_for("work_emballage"))
        ...
        rows = db.execute(
            ...
            SELECT i.*, l.code AS loc_code
            FROM item i LEFT JOIN location l ON i.location_id=l.id
            WHERE i.status IN ('EMBALLAGE')
            ORDER BY i.created_at ASC
            """
        ).fetchall()
        sol_slots = free_slots("SOL")
        etg_slots = free_slots("ETAGERE")
    ...
    def to_poste(item_id, poste_code):
        ...
        if not poste or poste["kind"] != "POSTE":
            abort(400)
        move_item(item_id, poste["id"], action=f"TO_{poste_code}")
        flash(f"Envoyé à {poste['name']}", "ok")
        return redirect(url_for("item_detail", item_id=item_id))
    ...
    def locations():
        ...
        if kind in ("SOL", "ETAGERE", "POSTE"):
            locs = db.execute(
                ...
                FROM location l WHERE kind=? ORDER BY code
                """,
                (kind,),
            ).fetchall()
        else:
            locs = db.execute(
                """
                SELECT l.*, (SELECT COUNT(*) FROM item i WHERE i.location_id=l.id) AS occ
                FROM location l ORDER BY kind, code
                """
            ).fetchall()
